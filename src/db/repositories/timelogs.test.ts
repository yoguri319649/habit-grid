...
describe('timeLogRepository', () => {
  ...
  it('addTimeLogは該当日のレコードがなければ新規作成する', async () => {
  ...
  it('addTimeLogは同日に複数回呼ぶと分数を加算する(上書きしない)', async () => {
    const task = await taskRepository.createTask('引っ越し');

    await timeLogRepository.addTimeLog(task.id, '2026-08-09', 30);
    await timeLogRepository.addTimeLog(task.id, '2026-08-09', 15);
  ...
  it('deleteTimeLogは対象レコードを削除する', async () => {
    const task = await taskRepository.createTask('引っ越し');
    const log = await timeLogRepository.addTimeLog(task.id, '2026-08-09', 30);
  ...
  it('listTimeLogsは日付の昇順で返す', async () => {
    ...
    await timeLogRepository.addTimeLog(task.id, '2026-08-09', 15);
    await timeLogRepository.addTimeLog(task.id, '2026-08-01', 30);
    await timeLogRepository.addTimeLog(task.id, '2026-08-05', 60);
